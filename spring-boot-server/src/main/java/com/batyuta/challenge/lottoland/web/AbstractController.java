 ...
 */
abstract class AbstractController<T extends BaseEntity<T>, V extends BaseVO> {
   ...
   */
  protected AbstractController(final AbstractEntityService<T> entityService,
   ...
   */
  protected Pageable buildPageable(final Integer page, final Integer size,
      final Sort sort) {
  ...
  public EntityModel<V> save(final @RequestBody V view) {
    ...
    return getEntityModel(lightweightService.toView(service.save(entity)),
        Pageable.unpaged());
  }
  ...
  public EntityModel<V> put(final @RequestParam Long id,
      final @RequestBody V view) {
    T entity = lightweightService.toEntity(view);
    entity.setId(id);
    return getEntityModel(lightweightService.toView(service.replace(entity)),
        Pageable.unpaged());
  }
  ...
  public EntityModel<V> patch(final @PathVariable Long id,
      final @RequestBody V view) {
    T entity = lightweightService.toEntity(view);
    entity.setId(id);
    return getEntityModel(lightweightService.toView(service.update(entity)),
        Pageable.unpaged());
  }
   ...
   * @return entity model
   */
  protected EntityModel<V> getEntityModel(final V view,
      final Pageable pageSettings) {
    Long viewId = view.getId();
    EntityModel<V> model = EntityModel.of(view,
    ...
    model.add(
        linkTo(methodOn(this.getClass()).findAll(null, pageable.getPageNumber(),
            pageable.getPageSize(), pageable.getSort())).withRel("all"));
    return model;
  }
}
