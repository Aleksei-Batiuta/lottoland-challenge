...
class UserEntityTest {
    ...
    public void compareToTest(final UserEntity user1, final UserEntity user2, final Object expected) {
        try {
            int result = user1.compareTo(user2);
            assertEquals(String.format("Unexpected comparing result: {data: {user1: %s, user2: %s}", user1, user2),
                    expected, result);
        } catch (Exception actual) {
            assertNotNull(String.format("Expected result is null: {data: {user1: %s, user2: %s}", user1, user2),
                    expected);
            assertEquals(String.format("Unexpected result! {data: {user1: %s, user2: %s}", user1, user2),
