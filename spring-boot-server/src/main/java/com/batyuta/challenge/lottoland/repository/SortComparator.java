...
public class SortComparator<T extends Comparable<? super T>>
  ...
  private Comparator<T> getSortComparator(final Sort orders) {
    ...
    if (orders != null) {
      for (Sort.Order order : orders) {
        Comparator<T> sortOrderComparator = sortOrderComparator(order);
        if (comparator == null) {
   ...
   * @return comparator
   */
  private <P extends Comparable<? super P>> Comparator<T> sortOrderComparator(
    ...
    Comparator<T> comparator = Comparator
        .comparing(o -> propertyBean.getProperty(order, o), orderComparator);
