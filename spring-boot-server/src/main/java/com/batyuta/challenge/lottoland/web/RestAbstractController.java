...
public abstract class RestAbstractController<T extends BaseEntity<T>, V extends BaseVO> {
   ...
   */
  public RestAbstractController(
      final AbstractEntityService<T> entityService, final LightweightService<T, V> lightweight) {
  ...
  protected EntityModel<V> getEntityModel(final V view, final Pageable pageSettings) {
    ...
    EntityModel<V> model =
        EntityModel.of(view, linkTo(methodOn(this.getClass()).findById(viewId)).withSelfRel());
    ...
    model.add(
        linkTo(
                methodOn(this.getClass())
                    .findAll(
                        null, pageable.getPageNumber(), pageable.getPageSize(), pageable.getSort()))
            .withRel("all"));
    return model;
  }
}
