...
public abstract class LockedData {
     ...
     * @return object
     */
    private <T> T lockedOperation(final Lock readOrWriteLock, final Supplier<T> command) {
        try {
            boolean isLockAcquired;
            int trying = 0;
            do {
                isLockAcquired = readOrWriteLock.tryLock(WAITING_TIME, WAITING_TIME_UNIT);
                trying++;
