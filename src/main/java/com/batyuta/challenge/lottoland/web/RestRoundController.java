...
public class RestRoundController
        extends RestAbstractController<RoundEntity, RoundVO> {
    ...
    public Page<RoundVO> findAll(
            final @RequestHeader(value = CSRF, required = false)
                    UserEntity user,
        ...
        if (user == null) {
            return new PageImpl<>(
                    ...
                    PageableUtils.fixPageable(pageable, 0),
                    0L
            );
        }
        Page<RoundEntity> allByUserId = roundService
