...
public class RestMessageControllerTest {
  ...
  public void getRequest(final String language, final Object expected) {
    ...
    try {
      ...
      if (expected != null) {
        assertEquals(
            String.format("Test case was failed: {data: {language: %s}", language),
            expected,
            actual);
      }
    } catch (Exception actual) {
      assertNotNull(
          String.format("Expected result is null: {data: {language: %s}", language), expected);
