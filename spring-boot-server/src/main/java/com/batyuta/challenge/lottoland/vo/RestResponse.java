...
public final class RestResponse<T> {
  ...
  @JsonFormat(shape = JsonFormat.Shape.STRING,
      pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS", timezone = "UTC")
   ...
   * @param httpStatus response HTTP status
   */
  private RestResponse(final T tBody, final HttpHeaders httpHeaders,
  ...
  public static RestResponse<ErrorBody> internalServerError(
      final Throwable ex) {
    return new RestResponse<>(new ErrorBody(ex),
   ...
   * @return response wrapper
   */
  public static RestResponse<ErrorBody> badRequest(final String message,
      final Throwable ex) {
    return new RestResponse<>(new ErrorBody(message, ex),
