...
public class StatusEnumTest extends AbstractTest {
  ...
  public static Stream<? extends Arguments> testData() {
    return Stream.of(Arguments.of(SignEnum.Const.WIN, StatusEnum.WIN),
        ...
        Arguments.of(TEST_STATUS_INVALID_NEGATIVE,
            new IllegalArgumentException()),
        Arguments.of(TEST_STATUS_INVALID_POSITIVE,
            new IllegalArgumentException()));
  }
  ...
  public void test(final int data, final Object expected) {
    try {
      ...
      assertEquals(String.format("Test case was failed: {data = %s}", data),
          expected, actual);
    } catch (IllegalArgumentException actual) {
      assertNotNull(String.format("Expected result is null: {data = %s}", data),
          expected);
