...
public class JsonConverter {
  /** The module for {@link HttpStatus} custom deserialization. */
  ...
  private static final ObjectMapper JSON_MAPPER =
      new ObjectMapper().findAndRegisterModules().registerModule(MODULE);

  /**
   * Parses the source string into object.
   *
   * @param source source string
   * @param aClass output object class type
   * @param <T> output object type
   * @return parsed object from JSON string
   */
   ...
   *
   * @param source source string
   * @param typeReference output object source type
   * @param <T> output object type
   * @return parsed object from JSON string
   */
  public <T> T toObject(final String source,
   ...
   * @return String view of message
   */
  private String toString(final Object object, final boolean pretty,
      final int depth) {
    if (object != null) {
      try {
        ...
        if (object instanceof LogEntryData && depth == 0) {
          ...
          if (args != null) {
            args.forEach(
                (key, value) -> result.put(key, toString(value, pretty, 1)));
            args.putAll(result);
          }
  ...
  /** Deserializer implementation for {@link HttpStatus} class. */
  static class HttpStatusDeserializer extends StdDeserializer<HttpStatus> {
    /** Default constructor. */
    protected HttpStatusDeserializer() {
    ...
    public HttpStatus deserialize(final JsonParser p,
      ...
      int id =
          (Integer) ((IntNode) node.get(HttpStatusSerializer.FIELD_CODE_VALUE))
              .numberValue();
