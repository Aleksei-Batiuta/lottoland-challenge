...
class UserEntityTest {
  ...
  public static Stream<? extends Arguments> testData() {
    return Stream.of(
        Arguments.of(new UserEntity("name1"), null, 1),
  ...
  public void compareToTest(final UserEntity user1, final UserEntity user2, final Object expected) {
    try {
      ...
      assertEquals(
          String.format("Unexpected comparing result: {data: {user1: %s, user2: %s}", user1, user2),
          expected,
          result);
    } catch (Exception actual) {
      assertNotNull(
          String.format("Expected result is null: {data: {user1: %s, user2: %s}", user1, user2),
          expected);
      assertEquals(
          String.format("Unexpected result! {data: {user1: %s, user2: %s}", user1, user2),
          expected.getClass(),
          actual.getClass());
    }
  }
