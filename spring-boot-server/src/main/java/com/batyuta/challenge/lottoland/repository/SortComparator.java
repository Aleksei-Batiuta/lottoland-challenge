 ...
 *
 * @param <T>
 *            Objects type class to compare
 */
// @Component("sortComparator")
public class SortComparator<T extends Comparable<? super T>> implements Comparator<T> {
    ...
    private Comparator<T> getSortComparator(final Sort orders) {
        ...
        if (orders != null) {
            for (Sort.Order order : orders) {
                Comparator<T> sortOrderComparator = getSortOrderComparator(order);
                if (comparator == null) {
     ...
     *            order type
     * @param <P>
     *            compared field type class
     *
     * @return comparator
     */
    private <P extends Comparable<? super P>> Comparator<T> getSortOrderComparator(final Sort.Order order) {
        ...
        Comparator<T> comparator = Comparator.comparing(o -> propertyBean.getProperty(order, o), orderComparator);
