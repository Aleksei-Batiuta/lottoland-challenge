...
public class RoundEntityRepository extends BaseEntityRepository<RoundEntity>
   ...
   */
  @LogEntry
  public Page<RoundEntity> getRounds(final Long userId, final Boolean isDeleted,
      final StatusEnum status, final Pageable pageSettings) {
    ...
    if (isDeleted != null) {
      roundsStream =
          roundsStream.filter(round -> round.isDeleted() == isDeleted);
    }
    if (status != null) {
